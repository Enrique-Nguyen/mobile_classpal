...
class LeaderboardService {
        ...
        .asyncMap((achievementSnapshot) async {
          ...
          for (final doc in achievementSnapshot.docs) {
            ...
            pointsByMember[achievement.memberUid] =
                (pointsByMember[achievement.memberUid] ?? 0) +
                achievement.points;
  ...
  static Stream<List<LeaderboardEntry>> streamTopEntries({
    ...
    return Stream.fromFuture(getCurrentLeaderboard(classId)).asyncExpand((
      leaderboard,
    ) {
      if (leaderboard == null) return Stream.value(<LeaderboardEntry>[]);
